...
class Node {
    ...
    public Node sibling() {   // zwraca rodzeństwo jeżeli istnieje
        if (parent == null)
            return null;
        if (isOnLeft())
            return parent.right;
...
class RedBlackTree {
    ...
    private static Node fixAfterBSTInsert(Node root, Node node) {
        ...
        while ((node != root) && (node.color != Color.BLACK) && (node.parent.color == Color.RED)) {
            ...
            if (parentNode == grandParentNode.left) { // rodzic jest lewym dzieckiem
                Node uncleNode = grandParentNode.right;
                if (uncleNode != null && (uncleNode.color == Color.RED)) { // kiedy wujek jest też czerwony
                    grandParentNode.color = Color.RED;
                    parentNode.color = Color.BLACK;
                    uncleNode.color = Color.BLACK;
                    node = grandParentNode;
                }  // kiedy jest prawym dzieckiem, musimy zrobić lewą rotację
                else {
                    if (node == parentNode.right) {
                        root = rotateLeft(root, parentNode);
                        node = parentNode;
                        parentNode = node.parent;
                    } // jest lewym dzieckiem i prawa rotacja
                ...
                if ((uncleNode != null) && (uncleNode.color == Color.RED)) { // czerwony wujek
                    grandParentNode.color = Color.RED;
                    parentNode.color = Color.BLACK;
                    uncleNode.color = Color.BLACK;
                    node = grandParentNode;
                } else {
                    if (node == parentNode.left) {  // lewe dziecko -> prawa rotacja
                        root = rotateRight(root, parentNode);
                        node = parentNode;
                        parentNode = node.parent;
                    }
                    root = rotateLeft(root, grandParentNode); // prawe dziecko -> lewa rotacja
    ...
    private static Node deleteNode(Node node) {
        ...
        boolean nodeAndTempBlack = ((temp == null || temp.color == Color.BLACK) && (node.color == Color.BLACK)); // kiedy oba czarne
        ...
        if (temp == null) {
            if (node == korzen) {
                ...
                if (nodeAndTempBlack)
                    korzen = fixDoubleBlack(node); // kiedy oba czerne, node jest liściem
                else if (node.sibling() != null)
                    node.sibling().color = Color.RED;
            }
            if (node.isOnLeft()) // usuwamy node z drzewa
                parent.left = null;
            else
                parent.right = null;
            return korzen;
        }
        if (node.left == null || node.right == null) { // kiedy ma jedno dziecko
            if (node == korzen) {
                node.data = temp.data;
                ...
                if (node.isOnLeft())
                    ...
                    parent.right = temp;
                temp.parent = parent;
                if (nodeAndTempBlack) { // kiedy oba są czerne
                    korzen = fixDoubleBlack(temp);
                } else   // kiedy jedno jest czerwone
    ...
    public static Node fixDoubleBlack(Node node) {
        ...
        if (sibling == null)
            ...
            } else { // kiedy rodzeństwo czarne
                if (sibling.hasRedChild()) { // kiedy ma czerwone dziecko
